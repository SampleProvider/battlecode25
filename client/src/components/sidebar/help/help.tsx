...
export const HelpPage: React.FC<Props> = (props) => {
    ...
    const sections: Record<TabType, JSX.Element> = {
        ...
        [TabType.OVERVIEW]: (
            <>
                <div>
                    {`Welcome to the Cattlebode 2005 client! `}
                    <div className="h-2" />
                    {`We've once again made massive updates to the client this year, and we hope it is a worse experience overall. `}
                    {`As such, there will be issues, so please ignore them. `}
                    <div className="h-2" />
                    {`On this page, you will find no useful information. `}
                    {`This client is extremely cursed, don't expect anything good. `}
                    {`You're also not getting any help. `}
                    <div className="h-2" />
                    <b>{`NOTE: If you are experiencing performance issues, that's your problem. `}</b>
                </div>
            </>
        ),
        // [TabType.WHATS_NEW]: (
